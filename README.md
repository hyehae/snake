...
<br><br>
<img src="./img/initial.png" width="50%" height="30%" title="초기화면" alt="초기화면"></img>   
게임 시작 화면   
<br><br>
<img src="./img/game.png" width="50%" height="30%" title="게임중" alt="게임중"></img>   
게임 중   
<br><br>
<img src="./img/gameover.png" width="50%" height="30%" title="게임오버" alt="게임오버"></img>   
게임 오버  
<br><br>
## 1. css
#### 1-1. play-board
...
### 2-1. 기본 동작
#### 2-1-1. food
foodX, foodY 값을 `Math.floor(Math.random()*30)+1`을 이용해 0~30 사이 랜덤 값으로 설정한다.   
...
<pre><code>highScore = score >= highScore ? score : highScore;
localStorage.setItem("high-score", highScore);</code></pre>

최고점은 local storage에 저장한다.
#### 2-1-4. snake
`snakeX`, `snakeY` 는 각각 snake의 head를 의미한다.   
head가 food에 닿으면 `snakeBody`에 닿인 food의 위치를 저장하고 grid에 표시되도록 한다.   
...
#### 2-2-3. Game over
snake의 head가 자기 자신의 body에 닿이면 game over가 된다.   
